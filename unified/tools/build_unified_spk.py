...
ROOT=Path(__file__).resolve().parents[2];U=ROOT/"unified";VER="0.2.0-0001"
...
def main():
 ...
 for n,p in {"bin/veramesh_supervisor.py":U/"payload/bin/veramesh_supervisor.py","bin/veramesh_runtime_init.py":U/"payload/bin/veramesh_runtime_init.py","bin/run-veramesh-gateway.sh":U/"payload/bin/run-veramesh-gateway.sh","bin/run-verarelay.sh":U/"payload/bin/run-verarelay.sh","ui/config":U/"payload/ui/config","ui/index.html":U/"payload/ui/index.html","third_party/dsmctl-LICENSE":U/"third_party/dsmctl-LICENSE"}.items():q[n]=(p.read_bytes(),0o755 if n.startswith("bin/") else 0o644)
 s=q["bin/veramesh_lifecycle.py"][0].decode();o='PACKAGE_VERSION = "0.1.0-0017"'
 if s.count(o)!=1:raise ValueError("lifecycle anchor")
 q["bin/veramesh_lifecycle.py"]=(s.replace(o,'PACKAGE_VERSION = "0.2.0-0001"').encode(),0o755)

...
import argparse,grp,hashlib,json,os,pwd,shutil,signal,socket,stat,subprocess,sys,time,urllib.request
...
SERVER=ROOT/"target/relay/src/server.js"; SUPERVISOR=ROOT/"target/bin/veramesh_supervisor.py"; RP=17443; EP=17445; SCHEMA="VERAMESH_VERARELAY_ADOPTION_V1"
...
def runtime_status():
 try:v=json.loads(RSTATUS.read_text())
 except Exception as x:raise E(f"runtime status unavailable: {x}") from x
 if v.get("schema")!="VERAMESH_RUNTIME_STATUS_V1":raise E("unexpected runtime status schema")
 g=v.get("reload_generation")
 if type(g) is not int or g<0:raise E("runtime status missing reload generation")
 return v
...
def supervisor_pid():
 try:
  st=SUPPID.lstat()
  uid,_=package_owner()
  if not stat.S_ISREG(st.st_mode) or stat.S_IMODE(st.st_mode)!=0o600 or st.st_uid!=uid:raise E("unsafe supervisor pid file")
 ...
 try:
  argv=[x.decode(errors="replace") for x in Path(f"/proc/{pid}/cmdline").read_bytes().split(b"\x00") if x]
 except Exception as x:raise E(f"cannot inspect supervisor pid {pid}: {x}") from x
 if str(SUPERVISOR) not in argv:raise E(f"pid {pid} is not exact VeraMesh supervisor")
 try:os.kill(pid,0)
 except OSError as x:raise E(f"supervisor pid {pid} not live: {x}") from x
 s=runtime_status()
 if s.get("supervisor_pid")!=pid:raise E("runtime status supervisor pid mismatch")
 return pid
...
def reload_modules(expect_relay,t=30):
 ...
 before=runtime_status();pid=supervisor_pid();generation=before["reload_generation"]
 ...
 while time.monotonic()<end:
  ...
  try:
   s=runtime_status()
   if s.get("supervisor_pid")!=pid:raise E("supervisor identity changed during module reload")
   if s["reload_generation"]<=generation:
    time.sleep(.25);continue
   if s.get("reload_error"):raise E("supervisor reload error: "+str(s["reload_error"]))
   relay=s.get("modules",{}).get("relay",{})
   if relay.get("enabled") is expect_relay:
    if expect_relay and relay.get("state")=="RUNNING" and port(RP):return {"supervisor_pid":pid,"reload_generation":s["reload_generation"]}
    if not expect_relay and relay.get("state")=="DISABLED" and not port(RP):return {"supervisor_pid":pid,"reload_generation":s["reload_generation"]}
  except E:raise
  except Exception:pass
  time.sleep(.25)
 raise E(f"supervisor did not converge relay enabled={expect_relay}")

def verify_health_continuity(before,after):
 for k in ("version","bindAddress","port","publicListener","applicationAuthentication","nodeRole","protocolDomain"):
  if before.get(k)!=after.get(k):raise E(f"Relay health continuity mismatch: {k}")
 if before.get("audit")!=after.get("audit"):raise E("Relay audit continuity mismatch")
 if before.get("audit",{}).get("ok") is not True:raise E("standalone Relay audit was not healthy")
 if after.get("audit",{}).get("ok") is not True:raise E("unified Relay audit is not healthy")
 return {"status":"PASS","audit_head":after["audit"].get("head"),"audit_records":after["audit"].get("records")}

def verify_source_preserved(state_copy):
 src=Path(state_copy["source_resolved"])
 _,d,nf,nb=manifest(src)
 if (d,nf,nb)!=(state_copy["source_tree_sha256"],state_copy["files"],state_copy["bytes"]):raise E("standalone source state changed after verified copy")
 return {"status":"PASS","tree_sha256":d,"files":nf,"bytes":nb}
...
def rollback(was,r,why,unified_changed):
 ...
 if unified_changed:
  try:enable(False);r["rollback"]["module_reload"]=reload_modules(False);quarantine_copy(r)
...
def apply():
 ...
 try:
  ...
  enable(True);unified_changed=True;r["module_reload"]=reload_modules(True);ha=health();r["continuity"]=verify_health_continuity(hb,ha);r["source_preservation"]=verify_source_preserved(r["state_copy"])

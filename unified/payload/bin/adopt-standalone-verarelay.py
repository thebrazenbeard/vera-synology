...
def filehash(p):
 ...
 return h.hexdigest()
def manifest(root):
 if not root.is_dir() or root.is_symlink():raise E(f"unsafe state root {root}")
 e=[];nf=nb=0
 ...
 return e,d,nf,nb
def atomic(p,v):
 p.parent.mkdir(parents=True,exist_ok=True,mode=0o700);q=p.with_name("."+p.name+".new")
...
def cfg():
 ...
 return v
def enable(x):
 v=cfg();v["modules"]["relay"]["enabled"]=x;atomic(MOD,v)
def node():
...
def chownroot(p):
 ...
 os.chown(p,u.pw_uid,g)
 for x in p.rglob("*"):os.chown(x,u.pw_uid,g,follow_symlinks=False)
def preflight():
...
def copy(stamp):
 e,d,nf,nb=manifest(OLDVAR);MIG.mkdir(parents=True,exist_ok=True,mode=0o700);rd=MIG/f"verarelay-standalone-{stamp}";rd.mkdir(mode=0o700)
 atomic(rd/"source-manifest.json",{"schema":"VERAMESH_VERARELAY_SOURCE_MANIFEST_V1","source":str(OLDVAR),"tree_sha256":d,"files":nf,"bytes":nb,"entries":e})
 if NEWVAR.exists():
  if NEWVAR.is_symlink() or not NEWVAR.is_dir() or any(NEWVAR.iterdir()):raise E(f"unified Relay state not empty/safe: {NEWVAR}")
  NEWVAR.rmdir()
 st=NEWVAR.with_name("relay.adopt-staging-"+stamp);shutil.copytree(OLDVAR,st,copy_function=shutil.copy2)
 _,d2,nf2,nb2=manifest(st)
 if (d2,nf2,nb2)!=(d,nf,nb):raise E("state copy verification failed")
 st.rename(NEWVAR);chownroot(NEWVAR);pid=NEWVAR/"state/runtime.pid";removed=False
 if pid.exists():
  if pid.is_symlink() or not pid.is_file():raise E("unsafe stale runtime pid")
  pid.unlink();removed=True
 return {"record_dir":str(rd),"source_tree_sha256":d,"copy_tree_sha256_before_ephemeral_cleanup":d2,"files":nf,"bytes":nb,"stale_runtime_pid_removed":removed,"original_state_preserved":OLDVAR.is_dir()}
def restart():
 action("stop",NEW);waitpkg(NEW,False);waitport(EP,False);action("start",NEW);waitpkg(NEW,True);waitport(EP,True)
def inspect():
 c=cfg()
 return {"schema":SCHEMA,"mode":"INSPECT","standalone_package":status(OLD),"unified_package":status(NEW),"standalone_state_exists":OLDVAR.is_dir(),"unified_state_exists":NEWVAR.is_dir(),"unified_state_empty":NEWVAR.is_dir() and not any(NEWVAR.iterdir()),"relay_module_enabled":c["modules"]["relay"]["enabled"],"relay_port_open":port(RP),"edge_port_open":port(EP),"node":node()}
def rollback(was,r,why):
 r["rollback"]={"attempted":True,"reason":why,"errors":[]}
 try:enable(False);restart()
 except Exception as x:r["rollback"]["errors"].append("disable unified: "+str(x))
 if was:
  try:
...
def apply():
 ...
 hb=health();stamp=datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ");r={"schema":SCHEMA,"status":"IN_PROGRESS","migration_id":stamp,"before":b,"standalone_health_before":hb}
 try:
  action("stop",OLD);waitpkg(OLD,False);waitport(RP,False);r["state_copy"]=copy(stamp);preflight();r["unified_config_preflight"]="PASS"
  enable(True);restart();waitport(RP,True);ha=health()
  ...
  r.update({"status":"PASS","unified_relay_health":ha,"standalone_package_final":status(OLD),"unified_package_final":status(NEW),"edge_loopback":"PASS" if port(EP) else "FAIL","relay_loopback":"PASS" if port(RP) else "FAIL","standalone_state_preserved":OLDVAR.is_dir(),"unified_relay_enabled":True,"safe_to_uninstall_standalone_after_review":True})
  atomic(Path(r["state_copy"]["record_dir"])/"qualification.json",r);return r
 except Exception as x:
  r["status"]="FAIL";r["error"]=str(x);rollback(b["standalone_package"]["running"],r,str(x))
